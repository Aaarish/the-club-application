...
public class ClubMemberController {
    ...
    @PostMapping
    public ResponseEntity<AppResponse> addMember(@PathVariable String clubId, String userIdForMember, @AuthenticationPrincipal AuthUser user) throws ResourceNotFoundException, AccessDeniedException {
        return ResponseEntity.ok(memberFacadeService.addClubMember(clubId, userIdForMember, user.getAppUser().getUserId()));
    }

    @DeleteMapping("/{memberId}")
    public ResponseEntity<DeleteResponse> removeClubMember(@PathVariable String clubId, @PathVariable String userIdForMember, @AuthenticationPrincipal AuthUser user) throws AccessDeniedException, ResourceNotFoundException {
